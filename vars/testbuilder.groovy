...
def call(path){
        ...
        echo "${WORKSPACE}\\${path}"
        def build_json = JsonOutput.toJson(["Executor_Number": "1", 'Workspace_Path' : env.WORKSPACE+'\\build_temp' , 'Build_Spec':"test-build", 'Target': "My Computer" , 'Project_Path': "${WORKSPACE}\\${path}" , 'Workspace_Path': env.WORKSPACE+'\\build_temp', 'Auto_Version_Increment': "true".toBoolean()])
        echo build_json
